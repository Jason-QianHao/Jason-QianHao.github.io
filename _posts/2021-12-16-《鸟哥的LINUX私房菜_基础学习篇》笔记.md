...
![image-20220111133612615](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220111133612615.png)
...
- Linux的常见例行性工作
  ...
  - 新建locate数据库
    ...
    对文件名数据库`/var/lib/mlocate/`文件进行更新
...
另外，kill后面接的数字**默认是PID**，如果是bash的工作控制，则要使用`%jobnumber`形式。
...
- 设置程序初始nice值
  ...
  ![image-20220107150142119](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220107150142119.png)
...
![image-20220107151704286](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220107151704286.png)
...
- 安全上下文
  ...
  安全上下文有点类似`rwx`。进程的安全上下文存在内存中，文件的安全上下文放置到文件的inode内。

  进程的安全上下文使用`ps -Z`查看，文件的安全上下文使用`ls -Z`查看：

  ![image-20220107160341505](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220107160341505.png)
...
- 查看运行状态
  ...
  使用`getenforce`进行SELinux状态查看

  ![image-20220110124621230](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110124621230.png)

- 查看策略等信息

  使用`sestatus`进行SELinux状态查看。

  **SELinux的配置文件是`/etc/selinux/config`**

  ![image-20220110125019761](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110125019761.png)

  效果如图：

  ![image-20220110125038013](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110125038013.png)

#### SELinux启动与关闭

- SELinux的运行状态、策略等更改时，需要重新启动SELinux

- 启动SELinux，需要编辑配置文件`/etc/selinux/config`

  ![image-20220110163820510](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110163820510.png)

  配置完后，在内核文件查询是否已经开启。**如果要开启SELinux，不可以在kernel行出现selinux=0的字样。**

  ![image-20220110164047471](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110164047471.png)

- SELinux模式在enforcing和permissive之间切换`setenforce`

  ![image-20220110164317413](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110164317413.png)

  注意，`setenforce`无法在Disabled模式下面进行切换。
...
- 进程的安全上下文使用`ps -Z`查看
  ...
  ![image-20220110165137536](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110165137536.png)

- 文件的安全上下文使用`ls -Z`查看

  ![image-20220110165153206](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110165153206.png)

httpd这个进程拥有的httpd_t这个domain去读取index.html文件时，就会看是否和文件的type匹配。

#### 修改文件的安全上下文`chcon`

![image-20220110165330344](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110165330344.png)

#### 文件的默认安全上下文的查询与修改`semanage`

![image-20220110172110502](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110172110502.png)

#### 文件的安全上下文恢复默认值`restorecon`

![image-20220110165413096](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110165413096.png)
...
![image-20220110170352796](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110170352796.png)

#### 详细规则（布尔值）查询sesearch

![image-20220110171422652](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110171422652.png)

#### 布尔值状态查询getsebool

![image-20220110171749959](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110171749959.png)

#### 布尔值关闭或开启setsebool

![image-20220110171825521](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110171825521.png)
...
- `setroubleshoot`，将错误信息写入`/var/log/messages`
  ...
  ![image-20220110165755103](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110165755103.png)
...
- `audit`，将详细数据写入`/var/log/audit/audit.log`

  与`setroubleshoot`不同的是，`auditd`会将所有信息记录下来，不仅仅是错误信息。

  设置服务开启：

  ![image-20220110170059951](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110170059951.png)![image-20220110170108678](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110170108678.png)

  可以使用`audit2why`进行文件数据分析

  ![image-20220110170211945](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110170211945.png)
...
​	常驻在内存当中的进程都负责一些系统所提供的功能以服务用户各项任务，因此这些常驻进程就会被称为服务(service)。执行service的程序或进程称之为**daemon**。
...
- super_daemon，通过这个特殊的daemon统一管理其他服务
  ...
  ![image-20220110173218216](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110173218216.png)
  ...
  ![image-20220110173450041](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110173450041.png)
...
![image-20220110173927162](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110173927162.png)
...
- `service`命令启动
  ...
  ![image-20220110174804482](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110174804482.png)

  service分析daemon后的参数，再去`/etc/init.d/*`取正确的服务来执行

#### 由Super_daemon管理的启动

​	**super_daemon是由`xinetd`这个进程实现的**，它管理的服务的配置文件在`/etc/xinetd.d/*`下面，通过编辑服务文件的`disable`字段来启动。
...
​	**xinetd管理的服务，有个默认的配置文件`xinetd.conf`**。当启动的服务，没有设置配置文件里面的值时，则该配置文件的值就作为默认值。

![image-20220110175640107](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110175640107.png)

#### 设置开启立即启动服务

- 管理系统服务默认开启开启启动chkconfig

  ![image-20220110181651492](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110181651492.png)

- 图形界面管理模式ntsysv

  Red Hat系统独有，通过图像界面管理服务

  ![image-20220110181743827](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110181743827.png)

#### 自定义服务

定义好shell script文件后，放入`/etc/init.d/`下面。文件有些格式要求：

`chkconfig:[runlevels][启动顺序][停止顺序]`

![image-20220110182231354](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110182231354.png)

然后还是使用`chkconfig`。

![image-20220110181931860](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110181931860.png)

### 服务的查看

​	前面在`Linux进程-系统资源管理-跟踪网络netstat`已经介绍过。查看端口即可查看相关服务。

### 服务的防火墙管理

​	防火墙，是针对源IP或域进行允许或拒绝的设置，以决定该连接是否能够成功实现连接的一种方式。

​	**任何以xinetd管理的服务都可以通过`/etc/hosts.allow和/etc/hosts.deny`来设置防火墙**。通过TCP数据包分析出IP和Port（TCP Wrappers）并搭配这两个文件，就能决定连接是否可以进入主机。**服务必须支持TCP Wrappers的功能，才能受这两个配置文件的限制**。
...
- 查询程序是否支持TCP Wrappers
  ...
  若有`libwrap.so`这个函数库文件，即支持TCP Wrappers.

- 配置文件语法

  ![image-20220110180554365](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110180554365.png)

- hosts.allow的优先级高于host.deny

- 配置文件第一和第二字段的特殊符号：

  ![image-20220110180741836](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220110180741836.png)
...
- 源码
  ...
  由于有些源码文件较大，所以一般软件的源码都是打包和压缩为Tarball文件，扩展名一般为`*.tar.gzip`或`*.tar.bz2`。里面的文件通常包含：
...
- Linux的软件
  ...
  ![image-20220111152129217](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220111152129217.png)
...
全名是RedHat Package Manager，有如下**优点**：
...
- RPM管理的方式使用**数据库记录PRM文件的相关参数**，便于升级、删除、查询与验证。

### 软件默认安装路径

- 安装软件完毕后，软件相关信息被写入`/var/lib/rpm/`目录下的数据库文件中

- 软件内的文件的位置：

  ![image-20220111152654789](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220111152654789.png)

  一般情况：如果是应用软件：`/usr/local`；如果是系统相关：`/usr/sbin`,`/usr/bin`等。具体可以使用`rpm -ql softwarename`查看。

### RPM安装软件

安装软件必须先取得**root权限**。

![image-20220111152831791](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220111152831791.png)

其他的参数：

![image-20220111152942318](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220111152942318.png)

### RPM升级与更新

![image-20220111153209580](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220111153209580.png)
...
本质是去查询`/var/lib/rpm/`目录下的数据库文件。

![image-20220111153252618](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220111153252618.png)

### RPM卸载软件与重建数据库

卸载的时候，很容易发送软件依赖属性导致无法删除的情况。所以删除的时候一定要从**最上往下删除**。

```shell
rpm -e softwarename
```

在RPM安装、删除、升级等过程中，可能会破坏`/var/lib/rpm/`目录下的数据库文件。可以进行数据库重建：
...
![image-20220111154027037](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220111154027037.png)

### yum安装和升级软件

![image-20220111154109937](https://gitee.com/Object_Jason/my-pic-go/raw/master/img/image-20220111154109937.png)
